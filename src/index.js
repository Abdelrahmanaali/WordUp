...
const daily=()=>{const day=new Intl.DateTimeFormat("en-CA",{timeZone:"UTC"}).format(new Date());let h=0;for(const c of day)h=(h*31+c.charCodeAt(0))>>>0;return WORDS[h%WORDS.length]};
...
export class GameRoom extends DurableObject{
 ...
 async fetch(req){
  ...
  if(u.pathname.endsWith("/ws")){
   ...
   if(!p){if(s.mode==="daily"&&s.players.length>=1)return new Response("Daily challenge already has a player",{status:409});if(s.players.length>=2)return new Response("Room is full",{status:409});p={id:pid,nickname:name,ready:s.mode==="daily",guesses:[],connected:true};s.players.push(p)}else{p.nickname=name;p.connected=true}
   if(s.mode==="daily"&&s.status==="waiting"){s.status="playing";s.startedAt=Date.now()}
  ...
  if(u.pathname.endsWith("/init")){
   ...
   s.code=b.code;s.mode=b.mode==="daily"?"daily":"duel";s.timer=TIMER_OPTIONS.includes(+b.timer)?+b.timer:120;s.word=s.mode==="daily"?daily():randomWord();s.status=s.mode==="daily"?"playing":"waiting";s.startedAt=s.mode==="daily"?Date.now():null;await this.save(s);return json({ok:true})
  }
 ...
 async webSocketMessage(ws,msg){
  ...
  if(d.type==="ready"&&s.mode==="duel"&&s.status==="waiting"){p.ready=true;if(s.players.length===2&&s.players.every(x=>x.ready)){s.status="playing";s.startedAt=Date.now();await this.ctx.storage.setAlarm(s.startedAt+s.timer*1000)}await this.save(s);this.broadcast({type:"state",state:this.public(s)});return}
  if(d.type==="guess"&&s.status==="playing"){
   ...
   if(s.mode==="daily"&&p.guesses.length===6){s.status="ended";s.endedAt=Date.now();s.result={winnerId:null,winnerName:null,type:"loss",reason:"Six guesses used — better luck tomorrow"};await this.save(s);await this.persist(s);this.broadcast({type:"state",state:this.public(s)});return}
   if(s.mode==="duel"&&s.players.length===2&&s.players.every(x=>x.guesses.length>=6)){s.status="ended";s.endedAt=Date.now();s.result={winnerId:null,winnerName:null,type:"draw",reason:"Both players used all 6 guesses"};await this.save(s);await this.persist(s);this.broadcast({type:"state",state:this.public(s)});return}
 ...
 async alarm(){const s=await this.load();if(s.status!=="playing"||s.mode!=="duel")return;s.status="ended";s.endedAt=Date.now();s.result={winnerId:null,winnerName:null,type:"timeout",reason:"Time ran out"};await this.save(s);await this.persist(s);this.broadcast({type:"state",state:this.public(s)})}
...
export default{async fetch(req,env){const u=new URL(req.url);
 if(u.pathname==="/api/daily"&&req.method==="POST"){const b=await req.json().catch(()=>({})),name=nick(b.nickname);if(!name)return json({error:"Nickname is required."},400);const c=code(),stub=env.GAME_ROOMS.getByName(c);await stub.fetch(new Request("https://room/init",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code:c,mode:"daily"})}));return json({ok:true,code:c})}
